...
Solver for the atomic Kohn-Sham equation, including all-electron and
pseudo-potential calculation.

## 💪 Getting Started

`atomic-femdvr` solves the radial Kohn–Sham equation for an isolated atom on a
finite-element discrete-variable (FEM-DVR) basis. It supports both
all-electron calculations and norm-conserving pseudopotentials read from UPF
files.

### Python API

```python
from atomic_femdvr import PseudoAtomicInput, solve_pseudo_atomic

inp = PseudoAtomicInput(
    sysparams={"element": "S", "file_upf": "S.upf", "lmax": 3, "nmax": 2},
    solver={"h_min": 0.5, "h_max": 4.0, "Rmax": 30.0},
    dft={"x_functional": "GGA_X_PBE", "c_functional": "GGA_C_PBE"},
)
...
The `atomic_femdvr` console script is installed alongside the package:
...
See `examples/` for runnable input files.

## 🚀 Installation

<!-- Uncomment this section after your first ``tox -e finish``
The most recent release can be installed from
[PyPI](https://pypi.org/project/atomic_femdvr/) with uv:

```console
$ uv pip install atomic_femdvr
```

or with pip:

```console
$ python3 -m pip install atomic_femdvr
```
-->
...
1. Uses [bump-my-version](https://github.com/callowayproject/bump-my-version) to
   ...
   `src/atomic_femdvr/version.py`, and
