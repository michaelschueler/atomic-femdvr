...
def solve_schrodinger_local(
    ...
    for il, l in enumerate(lchannels):
        ...
        if solver.lower() == "full":
            ...
            eps_l, vect = la.eig_banded(
                H_mat_banded, lower=False, select="i", select_range=[0, nmax]
            )
...
def get_green_function(
    ...
    M_inv_grid = 1.0 / (1.0 - 0.5 * alpha**2 * (Veff_grid - eps))
    Bterm_grid = 0.5 * alpha**2 * M_inv_grid**2 * dVeff_dr
    lM_grid = l * (l + 1) / (2.0 * r_grid**2) * M_inv_grid
