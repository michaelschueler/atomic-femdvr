...
def exchange_correlation_potential(
    basis: FEDVR_Basis,
    rho: np.ndarray,
    rho_nlcc: np.ndarray | None = None,
    xc_functional: str = "",
    x_functional: str = "gga_x_pbe",
    ...
    if driver == "internal":
        ...
        if xc_functional not in available_functionals:
            raise ValueError(
                f"Unsupported exchange-correlation functional: {xc_functional}. "
                f"Available functionals: {available_functionals}"
            )

        # Use internal GGA functional implementation

        _exc, xc_data = gga_functional(xc_functional, rho_, drho_dr, alpha_x)
        V_xc_grid = xc_data[0]
        ...
        try:
            ...
            raise ImportError(
                "pylibxc is not installed. Please install it to use the libxc driver."
            ) from err
