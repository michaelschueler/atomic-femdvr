...
def get_guess(rgrid: np.ndarray, Z: float, l: int, num_states: int, a0: float) -> np.ndarray:
    ...
    H_diag = - Z / np.sqrt(rs**2 + a0**2) + l * (l + 1) / (2. * rs**2)
    H_diag += 1.0 / h**2
    ...
    return lam, phi0
#========================================================================================================
def solve_iterative(basis:FEDVR_Basis, Z:float, l:int, num_states:int, a0:float=1.0e-2, driver: str = 'lobpcg',
                 preconditioner:str | None = None, maxiter:int=5000, tol:float=1e-8):
    """
    ...
    if preconditioner == 'diag':
        ...
        def prec(x):
            du2, d, du, y, info = la.lapack.dgtsv(H_offdiag, H_diag, H_offdiag, x)
            return y
    else:
        def prec(x):
            return x
    ...
    lam, X = get_guess(r_grid, Z, l, num_states, a0)

    if driver == 'primme':
        # solve eigenvalue problem using PRIMME

        eigvals, eigvecs = primme.eigsh(H_op, num_states, M=P_op, sigma=eps0,
                                         which='SM', maxiter=10000, tol=tol)
